...
#define SHADOWMAP_END            0x80000000
...
int realmain(int argc, char *argv[])
{
    ...
    FILE *f = fopen(PREFIX "/dump", "wb");
    ssize_t written = fwrite((void *)0x7ff00000, 1, 1024 * 1024, f);
    fclose(f);
    printf("written = %zd\n", written);
...
{
    struct stat st;
    logfile = fopen(PREFIX "/kdumper.log", "at") ? : stdout;
    myprintf("-\n");
    if (time(NULL) < 0 + 15 * 60 && stat(PREFIX "/start", &st) == 0 && unlink(PREFIX "/start") == 0) {
        sync();
        if (stat(PREFIX "/start", &st)) {
            return realmain(0, NULL);
        }
    }
    myprintf("NOT running\n");
    return 0;
}
